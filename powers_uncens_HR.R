# POWERS COMPUTING FOR ALTERNATIVES WITH DIFFRENT TYPE OF HAZARD RATE
...
list_of_df_amount_of_H1_wins_HR <- foreach(i = 1:m, .packages = c("foreach", "data.table")) %dopar%{
  # H0
...
colNames_vec <- colnames(list_of_df_amount_of_H1_wins_HR[[1]])
...
Power_df <- sapply(colNames_vec,
                   function(colName){
                     lapply(colnames(Quantiles_DF), 
                            function(rowName){
                              computePower(list_of_df_amount_of_H1_wins_HR,
                                           distName = colName, 
